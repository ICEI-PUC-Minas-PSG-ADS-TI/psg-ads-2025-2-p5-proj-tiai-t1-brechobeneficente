...
export default function FormularioCliente() {
  ...
  const { adicionarCliente, editarCliente, carregando } = useContext(ClientesContext)
  ...
  useEffect(() => {
    try {
      if (cliente && typeof cliente === 'string' && cliente.includes('{')) {
  ...
  const salvarCliente = async () => {
    ...
    try {
      if (editando) {
        await editarCliente(novoCliente)
      } else {
        ...
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
          ...
          >
            <View style={styles.header}>
              <Feather name="user" size={48} color={cores.primary} />
